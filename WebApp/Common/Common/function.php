...
/**
 * 正则检测邮箱手机
 * @param  string $str   需要被验证的字符串
 * @param  string $type  选填验证属性
 * @return  检测结果  匹配失败返回false 成功返回匹配到的
 */
function testing($str,$type){
	switch ($type){
		case 'email'://邮箱
			$isMatched = preg_match('/\w[-\w.+]*@([A-Za-z0-9][-A-Za-z0-9]+\.)+[A-Za-z]{2,14}/', $str, $matches);
			(empty($matches)) ? $res = false : $res = $matches;
			break;
		case 'moebl'://手机号
 ...
 * 通过id检索城市名
 * @param  int    $id   城市id
 * @param  string $type  返回属性
 * @return 中文城市名 
 */
function city_find($type,$id){
...
function order_sn() 
{
	$order_sn = date('Ymd').substr( implode(NULL,array_map('ord',str_split(substr(uniqid(),7,13),1))) , -8 , 5);
	return $order_sn;
}
/**
 * 取上一步来源地址
...
function user_data($id){
	$data = M('member')->where(array('member_id'=>$id))->find();
	return $data;
}
 ...
 * 记录登录
 * */
function record($member_id,$browser){
	$m_id = $member_id;	//登录id
	$bro = $browser;		//浏览器信息
	$time = NOW_TIME;
	$ip = get_client_ip();
